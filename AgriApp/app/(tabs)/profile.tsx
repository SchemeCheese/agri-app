...
type TabKey = 'info' | 'orders' | 'vouchers' | 'reviews';
...
const menuItems: { key: TabKey; label: string; icon: React.ComponentProps<typeof FontAwesome>['name'] }[] = [
...
export default function ProfileScreen() {
  ...
  const fetchOrders = useCallback(async () => {
    ...
    try {
      const res = await api.get<OrderData[]>('/orders/my-orders', {
        headers: { Authorization: `Bearer ${accessToken}` },
      });
  ...
  useEffect(() => {
    ...
    fetchVouchers();
  }, [activeTab, accessToken, user, fetchVouchers]);
  ...
  useFocusEffect(
    ...
    }, [user, accessToken, activeTab, fetchProfile, fetchOrders, fetchVouchers, fetchReviews]),
  );
  ...
  const handleCreateReview = async () => {
    console.log('🔍 handleCreateReview called', { 
      hasOrder: !!reviewingOrder, 
      rating: reviewRating,
      comment: reviewComment.length,
      hasToken: !!accessToken 
    });

    if (!reviewingOrder || !accessToken || reviewRating === 0) {
    ...
    try {
      console.log('📤 Sending review POST request...');
      await api.post(
        '/reviews',
        {
  ...
  return (
    <ScreenContainer>
        ...
        <View className="px-4 py-4 bg-[#F1F5F9]">
          ...
          {activeTab === 'info' ? (
            <View className="bg-white rounded-2xl border border-slate-100 mt-4 overflow-hidden">
              <View className="px-5 py-4 border-b border-slate-100 flex-row items-center justify-between">
                <View>
                  <Text className="text-slate-900 text-3xl font-black">Ho so ca nhan</Text>
                  <Text className="text-slate-400 mt-1">Thong tin tai khoan cua ban</Text>
                </View>
                {loadingProfile ? <ActivityIndicator size="small" color="#16A34A" /> : null}
              </View>
              ...
              <View className="p-5 gap-4">
                ...
                <View className="flex-row gap-3 mt-1">
                  <View className="flex-1 bg-emerald-50 border border-emerald-100 rounded-xl px-3 py-3">
                    <Text className="text-xs font-semibold text-emerald-700">San pham trong gio</Text>
                    <Text className="text-2xl font-black text-emerald-800 mt-1">{totalItems}</Text>
                  </View>
                  <View className="flex-1 bg-blue-50 border border-blue-100 rounded-xl px-3 py-3">
                    ...
                    <Text className="text-2xl font-black text-blue-800 mt-1">{orders.length}</Text>
                  </View>
                </View>
              </View>
            </View>
            ...
            <View className="mt-4">
              <View className="bg-white rounded-2xl border border-slate-100 px-5 py-4 mb-3 flex-row items-center justify-between">
                <View>
                  ...
                  <Text className="text-slate-400 mt-1">{orders.length > 0 ? `${orders.length} don hang` : 'Theo doi trang thai don hang'}</Text>
                </View>
              ...
              {!loadingOrders && orders.length > 0
                ? orders.map((order) => {
                    ...
                    const shopName =
                      order.seller?.profile?.store_name ||
                    ...
                    return (
                      <View key={order.id} className="bg-white rounded-2xl border border-slate-100 overflow-hidden mb-3">
                        ...
                        <View className="px-4 py-3 border-b border-slate-100 flex-row items-center justify-between">
                          <View className="flex-row items-center">
                            <FontAwesome name="home" size={12} color="#16A34A" />
                            <Text className="ml-2 text-slate-700 font-semibold">{shopName}</Text>
                          </View>
          ...
          {activeTab === 'reviews' ? (
            <View className="mt-4">
              <View className="bg-white rounded-2xl border border-slate-100 px-5 py-4 mb-3 flex-row items-center justify-between">
                <View>
                  <Text className="text-slate-900 text-3xl font-black">Danh gia cua toi</Text>
                  <Text className="text-slate-400 mt-1">
                    {reviews.length > 0 ? `${reviews.length} danh gia` : 'Xem lai tat ca danh gia da viet'}
                  </Text>
                </View>
              ...
              {!loadingReviews && reviews.length === 0 ? (
                <View className="bg-white rounded-2xl border border-slate-100 py-14 px-5 items-center">
                  ...
                  <Text className="text-slate-400 mt-1 text-center">Hay mua hang va chia se trai nghiem cua ban!</Text>
                </View>
              ) : null}
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              ...
              {selectedOrder?.status === 'COMPLETED' ? (
                <TouchableOpacity
                  ...
                  onPress={() => {
                    console.log('✅ Review button pressed', { orderId: selectedOrder?.id, status: selectedOrder?.status });
                ...
                <View>
                  <Text className="text-xs text-slate-400 text-center">
                    Status: {selectedOrder?.status} (Chi danh gia khi COMPLETED)
                  </Text>
                </View>
              )}
            </ScrollView>
      ...
      {/* Review Modal */}
      <Modal visible={reviewModalVisible} transparent animationType="slide" onRequestClose={() => {
        console.log('❌ Review modal close request');
        setReviewModalVisible(false);
      }}>
        <TouchableOpacity
          activeOpacity={1}
          onPress={() => {
            console.log('🚪 Backdrop pressed - closing review modal');
            setReviewModalVisible(false);
          }}
          className="flex-1 bg-black/50 justify-end"
        >
